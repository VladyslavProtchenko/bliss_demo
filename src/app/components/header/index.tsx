...
export default function Header() {
  ...
  const [openModileMenu, setOpenMobileMenu] = useState(false)

  const ref = useOnclickOutside(() => setOpen(false));
  const ref2 = useOnclickOutside(() => setOpen2(false));
  const ref3 = useOnclickOutside(() => setOpenMobileMenu(false));
    return (
      <header className={header}>
        <div className={wrapper}>
          ...
          <div onClick={()=>setOpenMobileMenu(!openModileMenu)} ref={ref3}>
            <RxHamburgerMenu className='text-[48px] md:hidden' />
            <nav className={openModileMenu ? mobileMenu : ' hidden '}  >
              {[...productMenu, ...solutionMenu].map(item =>
                <Link href={item.href} className={subItem} key={item.title}>
                  <h3 className={subHeader}>{item.title}</h3>
                  <p className={subText}>{item.text}</p>
                </Link>
...
const mobileMenu = 'absolute flex flex-col z-20 bg-white left-0 right-0 top-[80px]'
...
const wrapper = ' relative max-w-[1440px] flex w-full h-full items-center justify-between px-[40px] lg:px-[100px] '
